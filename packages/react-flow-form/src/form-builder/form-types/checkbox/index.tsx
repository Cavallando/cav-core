...
function Checkbox<FormValues extends FieldValues>({
  ...
  const { remove } = useFieldArray<FormValues>({
    control,
    name: formId as UseFieldArrayProps<FormValues, ArrayPath<UnPackAsyncDefaultValues<FormValues>>>["name"],
  });

  const handleChange = (option: { label: string; value: string }) => {
    let values = getValues(getValuesId);
    if (!values) {
      values = [] as PathValue<UnPackAsyncDefaultValues<FormValues>, Path<UnPackAsyncDefaultValues<FormValues>>>;
    }

    const indexFound = values.findIndex((item: { value: string }) => item.value === option.value);

    if (indexFound > -1) {
      remove(indexFound);
      const valuesToSave = values.filter((item: { value: string }) => item.value !== option.value);
      setItemsChecked(valuesToSave);
    } else {
      const valuesToSave = [...values, option as PathValue<FormValues, Path<FormValues>>];
      setItemsChecked(valuesToSave as PathValue<FormValues, Path<FormValues>>);
      setValue(
        getValuesId,
        valuesToSave as PathValue<UnPackAsyncDefaultValues<FormValues>, Path<UnPackAsyncDefaultValues<FormValues>>>
      );
    }
  };
    ...
    <CheckboxWrapper>
      {values.map((val) => (
        <CheckboxOption key={val.value}>
          <span>{val.label}</span>
          <input
            defaultChecked={itemsChecked.some((item: { value: string }) => item.value === val.value)}
